...
function create_tabs(args)
    if args.tabs then
        for _, v in ipairs(args.tabs) do
            if v.label == localize('b_full_deck') and SMODS.optional_features.amm_graveyard then
...
function G.UIDEF.view_graveyard()
    ...
    for i = #rank_name_mapping, 1, -1 do
		if type(SMODS.Ranks[rank_name_mapping[i]].in_pool) ~= "function" or SMODS.Ranks[rank_name_mapping[i]]:in_pool() then
